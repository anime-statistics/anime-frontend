...
function nextTagId(): string {
  const suffix = (mutableTags.length + 1).toString(16).padStart(2, '0')
  return `0f1a2b3c-4d5e-4f60-8a91-b2c3d4e5f6${suffix}`
}
...
export const tagHandlers = [
  ...
  http.delete(`${API_PREFIX}/tags/:id`, ({ params }) => {
    ...
    if (!mutableTags.some((tag) => tag.id === id)) {
      return new HttpResponse(null, { status: 404 })
    }

    mutableTags = mutableTags.filter((tag) => tag.id !== id)
